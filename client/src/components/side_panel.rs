...
use crate::sprites::{SPRITE_DIMENSION, SpriteRect, ITEM_SPRITE_DIMENSION};
...
pub fn SidePanelCharacterSheet(#[prop(into)] gs: Signal<Option<GameSnapShot>>) -> impl IntoView {
    view! {
        <aside class="w-80 bg-panel shadow-lg text-sm overflow-y-auto max-h-[720px]">
            ...
            <div class="divide-y divide-gray-700">
                {move || {
                    ...
                    gs.party
                        ...
                        .map(|player| {
                            view! {
                                <div class="flex items-center gap-2 p-2 bg-stone-900 border border-amber-800 mpt-0">
...
fn GoldCoinCanvas() -> impl IntoView {
    ...
    Effect::new(move |_| {
        ...
        let onload = Closure::<dyn FnMut()>::new(move || {
            draw_item_sprite(&ctx, &closure_coin_image, ITEMS_SPRITES.get("small_gold_pile"), 0.0, 0.0)
        });
