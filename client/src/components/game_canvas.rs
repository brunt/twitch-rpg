...
/// Struct for tracking smooth rendering state for an entity.
#[derive(Clone)]
...
pub fn GameCanvas(#[prop(into)] gs: Signal<Option<GameSnapShot>>) -> impl IntoView {
    ...
    // --- Effect 1: Handle snapshot updates ---
    let latest_snapshot_for_gs_effect = latest_snapshot.clone();
            ...
            *latest_snapshot_for_gs_effect.borrow_mut() = Some(snapshot.clone());
            ...
            if let Some(positions) = snapshot.floor_positions.as_ref() {
                ...
                for ep in positions {
                    ...
                    render_map
                        .entry(ep.id)
                        .and_modify(|re| re.target_pos = target)
    ...
    Effect::new(move |_| {
        ...
        let closure = Closure::<dyn FnMut()>::new(move || {
            ...
            let dt = ((now - anim.last_time) / 16.0).clamp(0.0, 1.0); // ~60fps normalized
            anim.last_time = now;
            ...
            if let Some(snapshot) = latest_snapshot_for_closure.borrow().as_ref() {
                if let (Some(floor), Some(cam_pos), Some(difficulty)) = (
                    snapshot.floor.as_ref(),
                    snapshot.camera_position.as_ref(),
                    snapshot.difficulty,
                ) {
                    // Smooth camera
                    let cam_x = cam_pos.x as f64;
                    let cam_y = cam_pos.y as f64;

                    // Smooth entities
                    {
                        let mut render_map = entity_render_map_for_closure.borrow_mut();
                        for re in render_map.values_mut() {
                            let smoothing_time_ms = 500.0;
                            let lerp_factor = 1.0 - (-dt / (smoothing_time_ms / 16.0)).exp();

                            re.render_pos.0 += (re.target_pos.0 - re.render_pos.0) * lerp_factor;
                            re.render_pos.1 += (re.target_pos.1 - re.render_pos.1) * lerp_factor;
                        }
                    }
