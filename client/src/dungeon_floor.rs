...
pub fn draw_dungeon_floor(
    ...
    // Compute where the camera tile would be drawn
    let camera_screen_x = (camera_col - camera_row) * (SPRITE_DIMENSION as f64 / 2.0)
        + canvas_width / 2.0
        - SPRITE_DIMENSION as f64 / 2.0;
    let camera_screen_y = (camera_col + camera_row) * (SPRITE_DIMENSION as f64 / 4.0);
    ...
    // Pick terrain set based on difficulty
    let [
        floor_tile,
        wall_tile,
        item_tile,
    ...
    // --- Draw entities using interpolated positions ---
            ...
            match re.entity_type.as_str() {
                ...
                cl => {
                    if let Ok(player_class) = PlayerClass::from_str(cl) {
                        let scale = if let Form::Scaled(s) = re.form {
                            s
                        } else {
                            1.0
                        };
