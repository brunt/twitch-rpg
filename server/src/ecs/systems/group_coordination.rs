...
impl<'a> System<'a> for GroupCoordination {
    ...
    fn run(&mut self, (
        ...
        let Some(adventure) = adventure.as_ref() else { return };
        let Some(dest_id) = group_destination.target_room_id else { return };
        let Some(room) = adventure.get_room_by_id(dest_id) else { return };

        let all_at_dest = (&players, &positions, &healths).join().all(|(_, position, health)| {
